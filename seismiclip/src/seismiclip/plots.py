...
def plot_random_image(images, W, H, seed=1234, **kwargs):
    """
    Plot a Numpy array as imshows of size height x width.

    :param array (_type_): Numpy array of size (NxWxH).
    :param height (_type_): Height of the image.
    :param width (int, optional): Width of the image. Defaults to 4.
    :param name (_type_, optional): File name. Defaults to None.
    :param save_dir (_type_, optional): Save directory. Defaults to None.
    """
    
    # Get the total number of images
    total_images = images.shape[0]
    
    # Select random indices for the subset of images

    set_seed(seed)
    indices = np.random.choice(total_images, size=W*H, replace=False)
    ...
    for i, ax in enumerate(axs.flat):
        ...
        # Plot the image
        ax.imshow(image, cmap='jet', **kwargs)
